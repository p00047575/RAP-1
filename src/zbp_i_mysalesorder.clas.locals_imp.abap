CLASS lhc_mysalesorder DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS get_global_authorizations FOR GLOBAL AUTHORIZATION
      IMPORTING REQUEST requested_authorizations FOR mysalesorder RESULT result.

*    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
*      IMPORTING keys REQUEST requested_authorizations FOR mysalesorder RESULT result.
*
*    METHODS get_features FOR FEATURES
*      IMPORTING keys REQUEST requested_features FOR MySalesOrder RESULT result.
...
*      READ TABLE lt_mysalesorders INTO DATA(ls_mysalesorder) WITH KEY SalesOrderID = ls_entity-SalesOrderID.
      IF sy-subrc = 0.
...
*    FAILED failed.
*
*    LOOP AT lt_mysalesorders INTO DATA(ls_mysalesorder).
*      APPEND VALUE #( LET upd_auth = if_abap_behv=>auth-allowed
*
*                                del_auth = if_abap_behv=>auth-allowed
*                                   crea_auth = if_abap_behv=>auth-allowed
*                            IN
*                             %tky = ls_mysalesorder-%tky
*                             %update                = upd_auth
...
*      ENTITY mysalesorder
*        ALL FIELDS WITH CORRESPONDING #( keys )
*      RESULT DATA(mysalesorders).
*
*    result = VALUE #( FOR mysalesorder IN mysalesorders
*                        ( %tky   = mysalesorder-%tky
*                          ) ).
*
*  ENDMETHOD.
*
...
*    LOOP AT entities INTO DATA(entity) WHERE salesorderid IS NOT INITIAL.
*      APPEND CORRESPONDING #( entity ) TO mapped-mysalesorder.
*    ENDLOOP.
*
*    DATA(entities_wo_salesorderid) = entities.
*    DELETE entities_wo_salesorderid WHERE salesorderid IS NOT INITIAL.
*
*    "Get max travelID
*    SELECT SINGLE FROM zvbak FIELDS MAX( id ) INTO @DATA(max_id).
*
*    " select from draft table
*    SELECT SINGLE FROM zdvbak FIELDS MAX( salesorderid ) INTO @DATA(max_id_draft).
*
*    IF max_id < max_id_draft.
*      max_id = max_id_draft.
*    ELSEIF max_id = 0.
*      max_id += 1.
*    ENDIF.
...
*
*    " Set Travel ID
*    LOOP AT entities_wo_salesorderid INTO entity.
*      max_id += 1.
*      entity-salesorderid = max_id .
*
